const points = [
  ...
  ,{ title: 'Квартира героя', subtitle: 'Тихий уголок после тяжёлого дня', kicker: 'Точка 9 · Одиночество', x: 84, y: 46, symbol: '⌂', question: 'Что особенно подчёркивает одиночество Акакия Акакиевича?', answers: ['Отсутствие близких людей и замкнутая жизнь среди служебных бумаг', 'Большая семья и постоянные гости', 'Популярность героя в светском обществе'], explanation: 'Герой почти не имеет личной жизни и существует в узком мире службы и переписывания.' },
  { title: 'Полицейский участок', subtitle: 'Место, где исчезает надежда на защиту', kicker: 'Точка 10 · Равнодушие', x: 88, y: 61, symbol: '⚑', question: 'Почему поиски шинели не дают результата?', answers: ['Потому что система перекладывает ответственность и не видит в беде героя важного дела', 'Потому что герой сам отказался искать шинель', 'Потому что шинель нашлась в мастерской Петровича'], explanation: 'Никто не воспринимает потерю маленького человека как событие, требующее помощи.' },
  { title: 'Призрачный Петербург', subtitle: 'Финальная точка маршрута и легенды', kicker: 'Точка 11 · Возмездие', x: 92, y: 80, symbol: '✦', dark: true, question: 'Какой смысл получает фантастический финал повести?', answers: ['Призрак героя символически возвращает униженным людям чувство справедливости', 'Фантастика полностью отменяет социальную проблему повести', 'Герой возвращается к обычной службе и забывает о шинели'], explanation: 'Фантастический финал превращает личную трагедию в образ запоздалого возмездия.' }
];
...
const taskSets = [
  ...
  [
    { type: 'choice', label: 'Анализ', question: points[1].question, answers: points[1].answers, correct: 0, explanation: points[1].explanation },
  ...
  [
    { type: 'choice', label: 'Одиночество', question: 'Что особенно подчёркивает одиночество Акакия Акакиевича?', answers: ['Отсутствие близких людей и замкнутая жизнь среди служебных бумаг', 'Большая семья и постоянные гости', 'Популярность героя в светском обществе'], correct: 0, explanation: 'Герой почти не имеет личной жизни и существует в узком мире службы и переписывания.' },
    { type: 'truth', label: 'Проверка деталей', question: 'Какие детали соответствуют повести?', statements: [{ text: 'Герой живёт очень замкнуто.', correct: true }, { text: 'Его окружают влиятельные друзья.', correct: false }, { text: 'Служебная рутина заменяет ему личную жизнь.', correct: true }], explanation: 'Замкнутость и служебная рутина делают одиночество героя особенно заметным.' }
  ],
  [
    { type: 'choice', label: 'Равнодушие', question: 'Почему поиски шинели не дают результата?', answers: ['Потому что система перекладывает ответственность и не видит в беде героя важного дела', 'Потому что герой сам отказался искать шинель', 'Потому что шинель нашлась в мастерской Петровича'], correct: 0, explanation: 'Никто не воспринимает потерю маленького человека как событие, требующее помощи.' },
    { type: 'map', label: 'Последняя остановка', question: 'Где герой ищет защиты после ограбления?', options: ['Полицейский участок', 'Магазин ткани', 'Праздничный вечер'], correct: 0, explanation: 'После ограбления герой пытается обратиться к официальной системе за помощью.' }
  ],
  [
    { type: 'choice', label: 'Фантастический финал', question: 'Какой смысл получает фантастический финал повести?', answers: ['Призрак героя символически возвращает униженным людям чувство справедливости', 'Фантастика полностью отменяет социальную проблему повести', 'Герой возвращается к обычной службе и забывает о шинели'], correct: 0, explanation: 'Фантастический финал превращает личную трагедию в образ запоздалого возмездия.' },
    { type: 'identity', label: 'Финальный образ', question: 'Кого встречают на петербургских улицах после смерти героя?', prompt: 'Таинственная фигура, отнимающая шинели у прохожих.', answers: ['Призрак Акакия Акакиевича', 'Петрович', 'Директор департамента'], correct: 0, explanation: 'Призрак становится фантастическим продолжением истории героя и символом возмездия.' }
  ]
];
...
function renderMarkers() {
  markerLayer.innerHTML = points.map((point, index) => {
    ...
    const state = isDone ? 'done' : 'active';
    const label = isDone ? '✓' : '●';
    return `<button class="marker ${state}" data-index="${index}" style="left:${point.x}%;top:${point.y}%" aria-label="${point.title}. ${point.subtitle}"><span class="marker-dot"><span class="symbol">${isDone ? '✓' : point.symbol}</span></span><span class="marker-number">${label} ${index + 1}</span><span class="marker-label">${point.title}</span></button>`;
  }).join('');
  ...
  progressText.textContent = `${finishedLocations} из ${points.length} мест`;
