...
function renderMarkers() {
  markerLayer.innerHTML = points.map((point, index) => {
    ...
    return `<button class="marker active" data-index="${index}" style="left:${point.x}%;top:${point.y}%" aria-label="${point.title}. ${point.subtitle}"><span class="marker-dot"><span class="symbol">${point.symbol}</span></span><span class="marker-number">● ${index + 1}</span><span class="marker-label">${point.title}</span></button>`;
  }).join('');
